"""Contact inequalities for two triples with an asymmetric collision and positive parameter."""
...
def contact_dd_variance(state):
    """Independent normalized curvature and direct event-variance Hessian."""
    put,get,inv=state.meet,state.get,state.inv
    A1,A2,h,v,V,G,z,phi,C,D=(get(k) for k in
        ('A1','A2','h','v','V','G','z','phi','C','D'))
    negative_part=(-z).max(arb(0))
    put('F',hi=1/(1+negative_part*negative_part))
    D=put('D',get('F')-get('Phi'))
    cap=get('rootV')*(15*D+(3*z-z**3)*phi)*inv('C')/12
    first=put('gamma1',hi=cap)
    second=put('gamma2',hi=cap)
    p=put('dd_drop_first',arb(3)/2-first,lo=0,hi=Q(3,2))
    q=put('dd_drop_second',arb(3)/2-second,lo=Q(-261,3136),hi=Q(3,2))
    for name in ('A1','A2'):put(name,hi=Q(4965,1568))
    put('gamma1',hi=Q(3,2))
    put('gamma2',hi=Q(4965,3136))
    put('m2',p*A1*h*inv('G'))
    p=put('dd_drop_first',G*get('m2')*inv('A1')*inv('h'))
    put('U',p*A1*h)
    put('r1',h*inv('A2')-q*h*v*inv('G'))
    put('q1',v*inv('A2')+q*h*v*inv('G'))
    put('F',get('e')*inv('A2')+get('m2')*q*h*v*inv('G'))
    q=put('dd_drop_second',G*(get('r1')-h*inv('A2'))*(-inv('h')*inv('v')))
    f1=put('event_variance_1',-get('m2')*inv('A1')*inv('h'))
    f2=put('event_variance_2',(get('r1')-h*inv('A2'))*inv('h')*inv('v'))
    f12=put('event_variance_12',inv('A1')*inv('h')**2*inv('v'),lo=0)
    normal=(3*D+(3*z+z**3)*phi)*inv('V')/4
    m11=put('minus_probability_H11',-3*f1-normal,lo=0)
    m22=put('minus_probability_H22',-3*f2-normal,lo=0)
    m12=put('minus_probability_H12',-V*f12-arb(3)*(f1+f2)/2-normal)
    put('probability_Hessian_determinant',m11*m22-m12*m12,lo=0)
...
def evaluate(case,box,*,sweeps=10,snapshot=False):
    ...
    try:
        k=put('k',x[0],lo=0,hi=1 if case=='negative' else Q(8,9))
        if case=='negative':
