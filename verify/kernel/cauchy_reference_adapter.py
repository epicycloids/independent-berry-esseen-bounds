...
def certificate(Llo, Lhi, box, eta):
    ...
    scale = 2 ** 64
    root = isqrt((dlo.numerator << 128) // dlo.denominator)
    require(Q(root * root, scale * scale) <= dlo < Q((root + 1) ** 2, scale * scale), 'Invalid rational square-root enclosure')
    wlo = dlo * Q(root, scale)
    excess = hi - taulo
    delta = lo * wlo - bhi * tauhi
    denominator = excess * (tauhi + RATIO * excess)
    require(0 < eta <= 1 and denominator > 0, 'Invalid reference contraction')
    if eta < 1:
        require(0 < delta <= denominator and eta ** 2 + (delta / denominator) ** 2 >= 1, 'No proved uniform positive-defect contraction on this rectangle')
    return dict(L=[str(lo), str(hi)], actual_box=list(map(str, map(Q, box))), eta=str(eta), sqrt_d_lower=str(Q(root, scale)), d_three_halves_lower=str(wlo), excess_upper=str(excess), defect_lower=str(delta), denominator_upper=str(denominator), price_over_Q=str(RATIO), contraction_margin=str(eta ** 2 + (delta / denominator) ** 2 - 1) if eta < 1 else None)
