"""Outward paired CF and cosine zero-bias refinements; companion proofs apply."""
import math
import time
import numpy as np
...
def rational_poly(z,coefficients):
    ...
    for c in coefficients[-2::-1]:p=add(mul(p,z),(al(c),au(c)))
    return p


def cos_abs_upper(xlo,xhi):
    # Both endpoint arguments are nonnegative; return the trivial bound
    # for a cell crossing 3, avoiding an unsupported monotonicity claim.
    safe_lo=np.clip(xlo,0,3);safe_hi=np.clip(xhi,0,3)
...
def paired_cells(t,dlo,dhi,bhi):
    ...
    term1=up(C*C);term2=up(up(up(C*excess)*t3)/3)
...
def cosine_cells(thi,Bhi,dhi,taulo,tauhi):
    tau=min(Bhi,max(0,taulo));x=up(thi*au(arb(dhi).sqrt()))
    q=up(1+up(up(x*x)/3));alpha=alpha_upper(x)
    excess=max(0,au(arb(Bhi)-arb(tau)))
    other=au(2*arb(Bhi)+2*arb(tau)/3)
    first=up(alpha*tau)
...
def checks():
    rng=np.random.default_rng(260929);cs=0;ps=0;zs=0
    for _ in range(100):
        v=float(rng.uniform(.0001,1));b=float(v**1.5*rng.uniform(1,3))
        ts=np.array([0.,.001,.1,.5,1.,2.,4.,10.]);x=ts*math.sqrt(v)
        C=cos_abs_upper(x,x)
        for xx,y in zip(x,C):assert arb(float(y))>=arb(float(xx)).cos().abs_upper();cs+=1
        out=paired_cells((ts,ts),v,v,b)
        for a,z,y in zip(ts[:-1],ts[1:],out):
            for tt in [a,(a+z)/2,z]:
                t=arb(float(tt));tau=arb(v)**(arb(3)/2);bb=arb(b)
                ...
                ps+=1
        tl=float(rng.uniform(0,b));th=float(rng.uniform(tl,b))
        out=cosine_cells(ts,b,v,tl,th)
        for tau in [tl,(tl+th)/2,th]:
            for tt,y in zip(ts,out):
                xx=arb(float(tt))*arb(v).sqrt();q=1+xx*xx/3
                alpha=2*xx/3-xx**3/15+xx**5/420 if xx<=2 else 2*xx/3
                tau0=arb(tau);bb=arb(b)
                ref=(alpha**2*tau0**2+q*q*(bb-tau0)*(2*bb+2*tau0/3)).sqrt()
                assert y==b or arb(float(y))>=ref,(v,b,tt,tau,y,ref)
                zs+=1
    return {'cosine_endpoint_enclosures':cs,'paired_cf_cell_checks':ps,
            'cosine_zero_bias_enclosures':zs,'all_passed':True}


def pilot():
    began=time.monotonic();check=checks();laws=finite_law_checks(CosineWeights,True);rows=[]
    for L,d,b,tau in [(.4,.195600146724,.092507500581,.376),
            (.5,.257461449454,.155637636600,.45),(.55,.299414822466,.191336229373,.495),
            (.6,.397750191359,.280850860934,.54),(.7,.454995299361,.359409523445,.595)]:
        for N in [256,512,1024]:
            bucket=min(20,max(1,math.ceil(20*tau/L)));w=CosineWeights(L,N,L*bucket/20);e=2e-12
            rows.append({'L':L,'d':d,'b':b,'tau':tau,'N':N,'T':w.T,'s':w.s,
                'parameter_halfwidth':e,'upper':w.box(L-e,L+e,d-e,d+e,b-e,b+e,tau-e,tau+e)})
    return {'status':'point enclosures and finite-law checks only','checks':check,
            'finite_laws':laws,'rows':rows,'elapsed_seconds':time.monotonic()-began}
...
def probe():
    ...
    def cosine(t,B,d,tau,directional=False):
        excess=max(0,B-tau);x=t*math.sqrt(d);q=1+x*x/3
        alpha=np.where(x<=2,2*x/3-x**3/15+x**5/420,2*x/3)
        return np.minimum(B,np.sqrt((alpha*tau)**2+q*q*excess*(2*B+2*tau/3)))
    log_probe.cf_single=paired_single;log_probe.stability=cosine
    result=log_probe.run_stable(True);result['refinements']=['paired CF','cosine zero bias']
    return result
