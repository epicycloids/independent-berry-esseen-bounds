...
from scalar_table_pilot import CutoffBatchWeights, CUTOFFS, LatestWeights
from scalar_table_bounds import table_cells, load_table
from further_bounds_pilot import certificate, quick_checks
from dual_log_bounds import dual_range, warmup, ORIGINAL
from signed_gaussian import verify_partition
import stable_bounds
import stable_cover
KIND = 'joint_scalar_dual5_cutoff'
PRODUCT = 'direct whole-log dual prices .75,1,1.5,2,3 with original baseline; complete joint complex scalar table with quadratic fallback; signed Gaussian fixed-cutoff portfolio'
...
class ScalarCutoffWeights(CutoffBatchWeights):
    prefactor = staticmethod(table_cells)

class ScalarLawWeights(LatestWeights):
    prefactor = staticmethod(table_cells)
...
def validation():
    ...
    try:
        ...
        finite_laws = stable_bounds.finite_law_checks(weights_class=ScalarLawWeights, extra=True)
    finally:
        stable_bounds.log_range_cells = old
    return dict(inherited=inherited, scalar=scalar, envelope=envelope, table=dict(rows=len(endpoints), last_endpoint=float(endpoints[-1]), price_coefficients=prices.tolist()), finite_laws=finite_laws, dual_support=warmup())
    ...
    def factory(hi, N, kind, target=None):
        ...
        def choose(tauhi):
            ...
            if bucket not in cache:
                weight = ScalarCutoffWeights(hi, N, hi * bucket / 20, cutoff_fractions=CUTOFFS)
