"""Outward bounds for a positive-sum Prawitz certificate.

Assumes IEEE-754 binary64, round-to-nearest elementary NumPy arithmetic
and sqrt, gradual underflow, and no unsafe fast-math transformation.
All transcendental values used below come from Arb or one-sided series.
"""
import math
import time
import numpy as np
...
def au(x):return float(up(float(arb(x).upper())))
def al(x):return float(down(float(arb(x).lower())))
...
def divpos(a,b):return down(a[0]/b[1]),up(a[1]/b[0])
def sqpos(a):return down(a[0]*a[0]),up(a[1]*a[1])
def cubepos(a):return mul(sqpos(a),a)


def exp_upper_from_nonnegative_lower(p):
    """Upper bound exp(-p), with p a nonnegative exact binary64 value."""
    p=np.maximum(0,p)
    x=np.minimum(p,25.)/64. # exact power-of-two division
    q=np.full_like(x,FACT_LO[16])
    for j in range(15,-1,-1):q=down(down(q*x)+FACT_LO[j])
    y=up(1/q)
    for _ in range(6):y=up(y*y)
    return np.minimum(1.,np.where(p>=25.,EXPCUT,y))


def one_minus_cos_lower(z):
    """Lower bound on 1-cos(z) for interval 0<=z<=2.3."""
    zz=sqpos(z)
    co=[arb((-1)**(j+1))/math.factorial(2*j) for j in range(1,7)]
    p=(al(co[-1]),au(co[-1]))
    for c in co[-2::-1]:p=add(mul(p,zz),(al(c),au(c)))
    return np.maximum(0,mul(p,zz)[0])


def psi_lower(t,V,E):
    """Lower bound on the exact Tyurin exponent at fixed V,E, t interval."""
    t2=sqpos(t);t3=mul(t2,t)
    cubic=sub(mul(ia(V/2),t2),mul((KLO,KHI),mul(ia(E),t3)))[0]
    if E<=0:return np.maximum(0,mul(ia(V/2),t2)[0])
    if V<=0:return np.zeros_like(t[0])
    arg=divpos(mul(ia(E),t),ia(V))
    valid=(arg[0]>=4.) & (arg[1]<=TWO_PI_LO)
    # Outside the valid cells the clipped argument is unused.
    z=(np.maximum(0,down(TWO_PI_LO-arg[1])),
       np.minimum(2.3,up(au(2*PI)-arg[0])))
    z=(np.minimum(z[0],2.3),np.clip(z[1],0.,2.3))
...
def f_upper(t,V,E,single=False):
    p=psi_lower(t,V,E)
    if single:return np.minimum(1.,up(np.sqrt(np.maximum(0,up(1-down(2*p))))))
...
def m_upper(t_hi,d):
    if d<=0:return np.zeros_like(t_hi)
    sq=arb(d).sqrt(); x=mul(ia(t_hi),(al(sq),au(sq)))
    x2=sqpos(x);x3=mul(x2,x)
    q=sub(mul(ia(.5),x2),mul((al(2*K),au(2*K)),x3))[1]
...
def log_product_cells(tlo,thi,Vlo,Vhi,Bhi,dhi,tauhi,deleted=False):
    """LOG_DEFECT.md, with whole-cell and whole-box coefficient bounds."""
    if Vlo<=0:return np.ones_like(tlo)
    t2lo=sqpos(ia(tlo))[0];t3lo=cubepos(ia(tlo))[0]
    t3hi=cubepos(ia(thi))[1]
    k=up(KHI*t3hi)
    sqrt_d=au(arb(dhi).sqrt());sqrt_V=au(arb(Vhi).sqrt())
...
class Weights:
    def __init__(self,Lmid,N=1024,log_correction=False):
        ...
        # These floats specify exact dyadic parameters, not approximations
        # required to bound any mathematical constant.
        self.T=float(round(math.pi/Lmid,8))
        self.s=float(min(.14+.52*Lmid,5/self.T))
        ...
        assert all(np.isfinite(x).all() for x in
                   [self.gauss,self.inner,self.low,self.high])
    ...
    def uniform(self,Llo,Lhi):
        ...
        E=au(2*arb(Lhi))
        B=exp_cubic_cells(self.tlo,self.thi,1.,E,d)
        F=f_cells(self.ht,1.,E)
        lowcf=f_cells(self.t,1.,E)
        return self.integrate(B,F,Llo,lowcf)

    def box(self,Llo,Lhi,dlo,dhi,blo,bhi):
        dl=arb(dlo);dh=arb(dhi)
        if bhi<al(dl*dl.sqrt()) or blo>Lhi:return None
        blo=max(blo,al(dl*dl.sqrt()))
        bhi=min(bhi,Lhi)
        if blo>bhi:return None
        whi=au(1-dl);wlo=max(0,al(1-dh))
        dh3=dh*dh.sqrt();dl3=dl*dl.sqrt()
        ...
        other=np.minimum(1.,up(A*D))
        def mixture(lam):return up(up(lam*R)+up(up(1-lam)*other))
        B=np.minimum(1.,np.maximum(mixture(lamlo),mixture(lamhi)))
        B=np.minimum(B,exp_cubic_cells(self.tlo,self.thi,1.,E,dhi))
        if self.log_correction:
            B=np.minimum(B,log_product_cells(self.tlo,self.thi,1.,1.,Lhi,dhi,tau,True))
        F=np.minimum(f_cells(self.ht,1.,E),
                     up(f_cells(self.ht,dlo,Ej,True)*f_cells(self.ht,wlo,ER)))
        lowcf=np.minimum(f_cells(self.t,1.,E),up(A*R))
        if self.log_correction:
            hl,hh=self.ht[0][:-1],self.ht[1][1:]
            Rhi=np.minimum(f_cells(self.ht,wlo,ER),log_product_cells(hl,hh,wlo,whi,BR,d2,tauR))
            F=np.minimum(F,np.minimum(up(f_cells(self.ht,dlo,Ej,True)*Rhi),
                                      log_product_cells(hl,hh,1.,1.,Lhi,dhi,tau)))
            lowcf=np.minimum(lowcf,log_product_cells(self.tlo,self.thi,1.,1.,Lhi,dhi,tau))
        return self.integrate(B,F,Llo,lowcf)


def elementary_checks():
    rng=np.random.default_rng(260906)
    p=np.r_[0.,np.geomspace(1e-20,100,300),rng.uniform(0,40,100)]
    e=exp_upper_from_nonnegative_lower(p)
    for x,y in zip(p,e):assert arb(float(y)) >= (-arb(float(x))).exp(),(x,y)
    z=rng.uniform(0,2.3,300)
    lower=one_minus_cos_lower((z,z))
    for x,y in zip(z,lower):assert arb(float(y)) <= 1-arb(float(x)).cos(),(x,y)
    for _ in range(80):
        V=float(rng.uniform(.001,1));E=float(rng.uniform(2*V**1.5,2)) if V<1 else 2.
        t=np.r_[rng.uniform(.001,15,16),4*V/E,2*math.pi*V/E]
        for single in [False,True]:
            out=f_upper((t,t),V,E,single)
            for x,y in zip(t,out):
                aa=arb(E)*arb(float(x))/arb(V)
                if aa<4:psi=arb(V)*arb(float(x))**2/2-K*arb(E)*arb(float(x))**3
                elif aa<2*PI:psi=arb(V)**3/arb(E)**2*(1-aa.cos())
                else:psi=arb(0)
                if psi<0:psi=arb(0)
                ref=(1-2*psi).sqrt() if single else (-psi).exp()
                # The exact modulus bound is at most one. Near a cosine
                # zero Arb can straddle one by its own enclosure radius.
                assert y==1. or arb(float(y))>=ref,str((V,E,x,single,y,ref))
    assert np.finfo(np.float64).bits==64 and np.nextafter(0.,1.)>0
    log_count=0
    for _ in range(120):
        V=float(rng.uniform(.05,1));d=float(rng.uniform(.001,V))
        B=float(rng.uniform(d**1.5,1.7));tc=float(min(B,math.sqrt(d)*V))
        tl=float(rng.uniform(.001,5));th=tl+float(rng.uniform(0,.02))
        for deleted in [False,True]:
            y=float(log_product_cells(np.array([tl]),np.array([th]),V,V,B,d,tc,deleted)[0])
            for tval in [tl,(tl+th)/2,th]:
                t=arb(tval);vv=arb(V);bb=arb(B);dd=arb(d);cap=arb(tc)
                mm=min(t*dd.sqrt(),1/(6*K));mm=mm*mm/2-2*K*mm**3 if deleted else arb(0)
                AA=t*t/2-2*K*dd.sqrt()*t**3;kk=K*t**3
                if AA>=0:
                    aa=AA/vv.sqrt()+kk;cc=kk*bb
                    tt=min(cap,(cc+max(mm,kk/(2*aa)))/aa)
                    qq=max(arb(0),aa*tt-cc)**2
                    penalty=max(arb(0),qq-mm*mm)
                else:tt=cap;penalty=arb(0)
                ref=(-vv*t*t/2+kk*bb+mm+kk*tt-penalty).exp()
                assert y==1. or arb(y)>=ref,str((V,B,d,tc,tl,th,deleted,y,ref))
                log_count+=1
    return {'exp_enclosures':len(p),'cosine_enclosures':len(z),'cf_enclosures':80*18*2,
            'log_enclosures':log_count,'all_passed':True,'arb_bits':ctx.prec,'numpy':np.__version__}


def pilot():
    start=time.monotonic();rows=[]
    checks=elementary_checks()
    for L,d,b in [(.2,.043,.009),(.3,.093,.029),(.4,.164,.0713),
                  (.48,.245,.127),(.5,.25,.133),(.55,.29,.18),(.7,.43,.33)]:
        for N in [512,2048]:
            w=Weights(L,N)
            old=w.box(L,L,d,d,b,b);w.log_correction=True
            rows.append({'L':L,'d':d,'b':b,'N':N,'T':w.T,'s':w.s,
                         'upper':old,'log_upper':w.box(L,L,d,d,b,b),'uniform':w.uniform(L,L)})
    return {'status':'point enclosures only; continuum cover not yet run',
            'checks':checks,'rows':rows,'elapsed_seconds':time.monotonic()-start}
