...
from energy_reference import energy_reference_bounds
from disk_reference_cover_engine import DiskReferenceWeights, selected, CUTOFFS
import stable_bounds

@lru_cache(maxsize=1)
def load_energy():
    path = Path(__file__).with_name('energy_table.json')
    if not path.is_file():
        path = Path(__file__).parent.parent / 'farfield_phase/new_target/energy/energy_table.json'
    result = json.loads(path.read_text())
    assert result['new_upper_interval_complete'] and result['verified_scalar_records'] == 64
    return result

class EnergyRectangles(SignedRectangles):
    ...
    def _reference(self, Llo, Lhi, dhi, taulo, tauhi):
        ...
        for j, nodes in enumerate(self.nodes):
            result = energy_reference_bounds(nodes, deleted[:, j], energy_table=load_energy(), L=(Llo, Lhi), d=(0.0, dhi), tau=(taulo, tauhi))
