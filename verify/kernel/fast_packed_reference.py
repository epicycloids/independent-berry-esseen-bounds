...
def fast_packed_energy_reference_bounds(nodes, deletion_caps, *, energy_table, L, d, tau, max_argument=Fraction(3, 2), precision_bits=128, prepared_prefix=None, prepared_geometry=None, prepared_box=None):
    ...
    result = dict(old)
    result.update(status='packed energy and reference component bounds', E_upper=radii, endpoint_upper=endpoints, cells=cells, old_E_upper=old['E_upper'], old_endpoint_upper=old['endpoint_upper'], energy_improved_source_cells=int(source_changes), energy_improved_radius_cells=sum((eligible and new < prior for eligible, new, prior in zip(old['eligible'], radii, old['E_upper']))), M_upper_role='Moment factor shared by the cellwise energy bounds', reference_source_sha256=frozen.REFERENCE_SHA256, energy_source_sha256=frozen.REVIEWED_SHA256, energy_table_replayed_per_box=False, small_frequency_source_sha256=energy_table['small_frequency_prefix']['source_sha256'])
    return result
