...
Then $V=K^{-2}$, $\tau=CK^3=C/V^{3/2}$, and the contact slope is $\sigma=\phi(z)/(CK^2)$.
...
By the contact-scale definition, $\sigma=\phi(z)/(CK^2)$.
Since $C<227/500$ and $K<8/5$,
\[
 \sigma>\frac{9/50}{(227/500)(8/5)^2}>\frac9{200}.
\]
Thus
...
When the selected representation uses the middle triple point, write the triple support as $-a,c,b$.
...
Before division by $K$, each coordinate has variance $1/2$.
Writing its middle mass as $m$,
...
derivative. The levels $(1,q,q)$ and gaps $Ku,K(a+b)$ give
...
The height bounds in \eqref{contact:collision-210-contact-reduction-4} follow from \eqref{contact:collision-210-contact-reduction-1}. They also bound the supports. If $b\ge 5/2$, then
...
with $H_{\ell},H_m\ge 0$, $\sigma=\ell H_{\ell}+m H_m>0$, and Bernoulli probabilities
$q,p>0$. The collision support equation is

\[
 q\ell H_{\ell}-pmH_m-3pq h\{1-h(p^2+q^2)\}=0.
\]
...
\[
 \boxed{
 m=\frac{q b H_{\ell}-3A p q h\{1-h(p^2+q^2)\}}
           {qvH_{\ell}+A pH_m}.}
\tag{C.17.1}\label{contact:collision-210-mass-elimination-1}
\]

The denominator $L=qvH_{\ell}+A pH_m$ is strictly positive: every coefficient
...
\[
 \mathcal D=G^3(NvH_{\ell}+A PH_m)=G^4 L,
\]
\[
 M=N bH_{\ell}G^3-3A PN hG^2+3A PN h^2W.
\tag{C.17.2}\label{contact:collision-210-mass-elimination-2}
\]

Then $m=M/\mathcal D$. The four natural mass conditions become polynomial:

\[
 M>0,\quad\mathcal D-M>0,\quad
 b\mathcal D-vM>0,\quad t\mathcal D-hM>0.
...
By \eqref{contact:fixed-c2-threshold-collar-2}, $\sigma>9/200>1/80$.
Since $\ell,m<1$, one has $H_{\ell}+H_m\ge \sigma$, and hence
...
\[
 \boxed{\mathcal D>\frac{65}{3170893824}>\frac1{50000000}.}
...
test; division is permitted only if the direct enclosure of $\mathcal D$ is
...
\[
 L_*=b\mathcal D-vM,\quad H_*=t\mathcal D-hM,
 \quad S_*=L_*H_{\ell}+A MH_m,
 \quad O=cN-tP,
\]
\[
 V_*=(tb\mathcal D-Mhv)G^2+\mathcal D PN h^2,
\]
\[
...
\[
 V=\frac{V_*}{\mathcal D G^2},\quad
 R=\frac{R_*}{A\mathcal D G^4},\quad
 \sigma=\frac{S_*}{A\mathcal D},\quad
 F=\frac{F_*}{A\mathcal D G},\quad
 z=O\sqrt{\mathcal D/V_*}.
...
\[
 A\mathcal D h(N-P)\{h^2(N-P)^2-3hG^2/2\}
                  -S_*hG^3+A MG^4=0,
\]
\[
...
\]
\[
 S_*^2V_*-A^2\mathcal D^3G^4\phi(z)^2=0,
\]
\[
 G^4\{F_*-A\mathcal D G\Phi(z)\}-R_*=0.
...
equations in the three support parameters on either sign chart.

The ratio test is
$(49/120)^2 \mathcal D^3 G^8\le V_*^3<(227/500)^2 \mathcal D^3 G^8$.
The added width tests are
$3bh \mathcal D G^2>V_*$ and $3tv \mathcal D G^2>V_*$.
...
support gaps are $u,v>0$, its actual centered unit-variance
CDF derivative is
...
\paragraph{Unit-span coordinates}
...
Let $p=(p_0,p_1,p_2)$ and $q=(q_0,q_1,q_2)$ be the positive
probability vectors of the two laws, with means $\mu,\nu$.
Put $m=p_1$ and $n=q_1$. Write
$x,y$ for their centered support vectors, $v_i,r_i$ for their
variances and third absolute moments, and
$M_1=\sum_jp_jx_j|x_j|$, $M_2=\sum_jq_jy_j|y_j|$. Set

\[
 V=v_1+v_2,\qquad R=r_1+r_2,\qquad o=1-\mu-\nu,
 \qquad z=o/\sqrt V.
\tag{C.23.2}\label{contact:three-owner-321-contact-reduction-3}
\]

\paragraph{Event and contact equations}
...
\[
 ...
 \qquad C=D V^{3/2}/R.
\tag{C.23.3}\label{contact:three-owner-321-contact-reduction-5}
\]
...
\tag{C.23.4}\label{contact:three-owner-321-contact-reduction-6}
\]
...
\[
 ...
 H_2=(1,p_0+p_1,p_0).
\tag{C.23.5}\label{contact:three-owner-321-contact-reduction-7}
\]
...
\tag{C.23.6}\label{contact:three-owner-321-contact-reduction-8}
\]
...
\tag{C.23.7}\label{contact:three-owner-321-contact-reduction-12}
\]
...
\[
 ...
 N_1N_2-H_{12}^2\geq0.}
\tag{C.23.8}\label{contact:three-owner-321-contact-reduction-13}
\]
...
In the three-representation $321$ case, retain the notation
...
\]
Hence $T>19/100$, so
\eqref{contact:three-owner-321-variance-upper-9} contradicts
$V\ge3/10$. Therefore $V<3/10$.
...
The two coordinate variance fractions exceed $13/200$. Define the piecewise cubic
function
...
\tag{C.28.4}\label{contact:two-owner-321-contact-reduction-11}
\]

\paragraph{Variance Hessian}
For either threshold geometry, take arbitrary positive mass vectors
$(\ell_i,m_i,r_i)$ on its centered supports. Let $v_i$ and $R_i$
be their variances and third absolute moments, and put
\[
 V=v_1+v_2,\qquad R=R_1+R_2,\qquad z=o/\sqrt V,\qquad
 F=1-m_1r_2-r_1(m_2+r_2).
\]
At the attained ratio, $\tau=C/V^{3/2}$ and $D=F-\Phi(z)=\tau R$.
...
\tag{C.28.5}\label{contact:two-owner-321-contact-reduction-21}
\]
...
$m_1=(A_1 g_e(d)/G-b)/u$. The five-parameter box retains $m_1$ and
...
But \eqref{contact:two-triple-221-fixed-moment-support-hessian-1} and \eqref{contact:two-triple-221-fixed-moment-support-hessian-9} also give $L(d)\ge 2Z/m_2=2H_d/u$, so
...
Again \eqref{contact:two-triple-221-negative-middle-location-2} is strictly below $L(d)-2H_d/v=3-6e<0$.
