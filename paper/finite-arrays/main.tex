...
and interval certificates. For a three-point law and two two-point laws
with a specified triangular threshold, we derive necessary conditions
for a maximum over the full independent class. The two Bernoulli
probabilities agree; the remaining masses are affine functions of total
variance; and a mixed second variation gives a quadratic inequality in
that variance. The computational supplement supplies interval
...
For a centered sum $S$ of variance $B^2$ and $\varepsilon\in\{-1,1\}$, define
\[
...
Sections~\ref{sec:two}--\ref{sec:equal-gap-two-point} prove the
...
Section~\ref{sec:triangular-contact} treats a threshold configuration
outside the settled two-summand and three-Bernoulli classes: one genuine
three-point coordinate and two two-point coordinates, with the three
threshold representations specified in
\eqref{eq:triangle-configurations}. Proposition~\ref{prop:triangle-equal-probabilities}
equalizes the two probabilities, and Theorem~\ref{thm:triangle-projection}
eliminates the masses in favor of the original variance. The resulting
normalization, density and curvature equations are necessary conditions
for an attained maximum over the full independent class with this
threshold pattern.

\section{Two Bernoulli summands with arbitrary diameters}\label{sec:two}
...
To see that the table covers every Bernoulli array, the two possible
...
satisfies $F_{\rm row}\le F_S^+(y)$. Thus a negative row deficit
implies a negative deficit for the actual law. The parameter bounds
derived under that hypothesis therefore apply in the row's
negative-set derivative test.
...
\end{theorem}
We minimize the signed deficit over the Bernoulli probabilities.
The stationarity equations reduce a hypothetical negative minimum
to a binomial law or to a binomial sum with one exceptional Bernoulli
coordinate. Schulz's theorem treats the first case. A curvature
calculation and the two-summand deficit treat the second.
...
When $M=1$, the two-summand deficit in
Theorem~\ref{thm:two-summands} gives the required bound.
...
Delete deterministic coordinates and divide every remaining summand
by the common support diameter. Each can then be written
...
For a centered three-point law with support $\{-a,c,b\}$, where
$a,b>0$ and $-a<c<b$, put
\[
 d=\min\{b(a+c),a(b-c)\}.
\]
This is the absolute difference between the variances of its two
centered two-point components; when $c=0$, one component is $\delta_0$.
Set $d=0$ for a law with at most two support points.

\begin{lemma}[Small variance width]\label{lem:mixture-width}
For two laws with at most three support points, let $d_i$ be the
nonnegative variance widths just defined. If
...
\input{three-coordinate-contact.tex}
