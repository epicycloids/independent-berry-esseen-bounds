...
divisible remainder and an additive third-moment quantity attached to the
representation. A maximizing representation can be chosen with
summands having at most three support points. Its Gaussian and L\'evy
components vanish, and the supports of its independent summands shrink
to zero. If infinitely many
...
maximizing threshold. This condition excludes infinitely many finite blocks
satisfying a stated separation condition. For a maximizing representation
with at most three support points per summand, total variance one, and
additive third-moment quantity at most $18481/25000$, we prove
$\sum_i\Var(X_i)/\diam(\supp X_i)^\gamma<\infty$ for
$0<\gamma<\sqrt5-1$, where the sum runs over nonconstant coordinates.
Consequently $\sum_i\E|X_i|<\infty$. If all but finitely many
nonconstant summands have two support points, the exponent improves to
$\sqrt{21}-3$. Under the same third-moment bound, the maximizing
distribution function cannot satisfy
$F(z+t)=F(z)+\phi(z)t+O(t^2)$ as $t\to0$ through both signs at a
maximizing threshold $z$. The sharp inequality for all independent arrays
remains open.
\end{abstract}
...
\cite[Theorem~1.1]{BellSmall2026}: with
$\ell_0=\exp(-\exp(20004))$,
...
\end{equation}
That theorem extends the uniform smoothing, contamination and local
comparison methods of He and Cheng \cite{HeCheng2026} to
nonidentically distributed arrays, with explicit constants. Its finite input
...
An admissible representation is a decomposition of the form
\begin{align}
 S&=\sum_{i\ge1}X_i+Y,\notag\\
 \log\E e^{itY}
   &=-\frac{at^2}{2}
...
\end{align}
Here the $X_i$ are independent centered variables, and $Y$ is a centered
infinitely divisible variable independent of all $X_i$. The number $a$ is
nonnegative, the series converges in $L^2$, and $\nu$ is a L\'evy measure
with finite second moment. The number $\beta$ is the additive
...
\begin{theorem}[Weighted variance of the original coordinates]
\label{thm:weighted-variance}
Suppose $\ci>\ce$, and let a maximizing representation as in
Theorem~\ref{thm:structure} have
$\beta\le18481/25000$. For each nonconstant coordinate put
$H_i=\diam(\supp X_i)$ and $v_i=\Var(X_i)$. Then
\[
 \sum_i\frac{v_i}{H_i^\gamma}<\infty
       \quad(0<\gamma<\sqrt5-1),\qquad
                         \sum_i\E|X_i|<\infty .
\]
If all but finitely many nonconstant coordinates have two support points, the
weighted-variance range improves to $0<\gamma<\sqrt{21}-3$.
These statements concern the original independent coordinates.
\end{theorem}

\begin{corollary}[Regularity excluded at a maximizing threshold]
\label{cor:weighted-regularity}
Under the hypotheses of Theorem~\ref{thm:weighted-variance}, a threshold
satisfying \eqref{eq:max-data} cannot obey
\[
             F(z+t)=F(z)+\phi(z)t+O(t^2)\qquad(t\to0)
\]
through both signs. If all but finitely many nonconstant coordinates
have two support points, it cannot obey
\[
       F(z)-F(z-h)=\phi(z)h+O(h^{1+\alpha})\qquad(h\downarrow0)
\]
for any $\alpha>(\sqrt{21}-3)/6$.
\end{corollary}

The third-moment bound supplies the two strict curvature inequalities in
Lemma~\ref{lem:weighted-scalar}; it is not a consequence of attainment.
Write $W_s=\sum_i v_i/H_i^s$, summing over nonconstant coordinates.
The proof first establishes $W_s<\infty$ for some $s>1$.
Bounds on the number of coordinates above a variance threshold then
permit more efficient grouping of the remaining coordinates. Gaussian
and lattice comparisons turn the grouping estimates into
$W_\gamma<\infty$ for every $0<\gamma<(3s+4)/(s+5)$.
Iteration reaches every exponent below the positive fixed point
$\sqrt5-1$. When all but finitely many nonconstant coordinates have
two support points, grouping by the sign of the nonmodal displacement
gives the map $(3s+12)/(s+9)$, whose positive fixed point is
$\sqrt{21}-3$.
Sections~\ref{sec:weighted-shape}--\ref{sec:weighted-iteration} give
the full argument.
...
when every relevant remainder has support $\R$. Under the additional
third-moment bound, Theorem~\ref{thm:weighted-variance} gives weighted
summability, and Corollary~\ref{cor:weighted-regularity} excludes the
stated local expansions of the distribution function.
...
\cite[Lemma~4.3]{HeCheng2026}; the independent-coordinate derivatives,
...
contamination argument of \citet[Lemma~4.3]{HeCheng2026}.
...
\input{variance-and-regularity.tex}
